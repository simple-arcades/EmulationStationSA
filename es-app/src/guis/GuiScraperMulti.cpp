	...
	mSearchQueue(searches)
{
	...
	// set up grid
	mTitle = std::make_shared<TextComponent>(mWindow, "SCRAPING IN PROGRESS", saFont(FONT_SIZE_LARGE), SA_TITLE_COLOR, ALIGN_CENTER);
	mGrid.setEntry(mTitle, Vector2i(0, 0), false, true);

	mSystem = std::make_shared<TextComponent>(mWindow, "SYSTEM", saFont(FONT_SIZE_MEDIUM), SA_TEXT_COLOR, ALIGN_CENTER);
	mGrid.setEntry(mSystem, Vector2i(0, 1), false, true);

	mSubtitle = std::make_shared<TextComponent>(mWindow, "subtitle text", saFont(FONT_SIZE_SMALL), SA_SCRAPER_SUBTITLE_COLOR, ALIGN_CENTER);
