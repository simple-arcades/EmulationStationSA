	...
	mTargetConfig(target), mHoldingInput(false), mBusyAnim(window)
{
	...
	mTitle = std::make_shared<TextComponent>(mWindow, "CONFIGURING", saFont(FONT_SIZE_LARGE), SA_TITLE_COLOR, ALIGN_CENTER);
	...
	mSubtitle1 = std::make_shared<TextComponent>(mWindow, Utils::String::toUpper(ss.str()), saFont(FONT_SIZE_MEDIUM), SA_TITLE_COLOR, ALIGN_CENTER);
	mGrid.setEntry(mSubtitle1, Vector2i(0, 2), false, true);

	mSubtitle2 = std::make_shared<TextComponent>(mWindow, "HOLD ANY BUTTON TO SKIP", saFont(FONT_SIZE_SMALL), SA_SUBTITLE_COLOR, ALIGN_CENTER);
	...
	for(int i = 0; i < inputCount; i++)
	{
		...
		icon->setColorShift(SA_INPUT_ICON_COLOR);
		icon->setResize(0, saFont(FONT_SIZE_MEDIUM)->getLetterHeight() * 1.25f);
		...
		auto text = std::make_shared<TextComponent>(mWindow, GUI_INPUT_CONFIG_LIST[i].dispName, saFont(FONT_SIZE_MEDIUM), SA_TEXT_COLOR);
		row.addElement(text, true);

		auto mapping = std::make_shared<TextComponent>(mWindow, "-NOT DEFINED-", saFontLight(FONT_SIZE_MEDIUM), SA_SUBTITLE_COLOR, ALIGN_RIGHT);
